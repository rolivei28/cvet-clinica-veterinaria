...
#### 3.2.1 Padrões GoF a evoluir na arquitetura MVC

- **Repository:** Hoje as rotas Hono (`internacoes.ts`, `financeiro.ts`) acessam o Prisma diretamente e duplicam a forma de `include` (pet/tutor/leito/medicações/pagamentos) entre arquivos. Proposta: extrair `InternacaoRepository`, `LeitoRepository`, `PetRepository` e `PagamentoRepository` em `backend/src/repositories/`, centralizando queries e includes e permitindo testar as rotas com um repositório fake (sem banco real).
